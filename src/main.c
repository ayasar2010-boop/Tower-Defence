...
Vector2 GridToWorld(int gx, int gy) {
    return (Vector2){GRID_OFFSET_X + gx * CELL_SIZE + CELL_SIZE / 2.0f,
...
bool WorldToGrid(Vector2 wp, int *gx, int *gy) {
    ...
    if (ix < 0 || ix >= GRID_COLS || iy < 0 || iy >= GRID_ROWS)
        return false;
    *gx = ix;
    *gy = iy;
    return true;
...
int GetTowerCost(TowerType type) {
    switch (type) {
    case TOWER_BASIC:
        return TOWER_COST_BASIC;
    case TOWER_SNIPER:
        return TOWER_COST_SNIPER;
    case TOWER_SPLASH:
        return TOWER_COST_SPLASH;
    default:
        return 999;
    }
}
...
bool CanPlaceTower(Game *g, int gx, int gy) {
    if (gx < 0 || gx >= GRID_COLS || gy < 0 || gy >= GRID_ROWS)
        return false;
    if (g->grid[gy][gx] != CELL_BUILDABLE)
        return false;
    if (g->gold < GetTowerCost(g->selectedTowerType))
        return false;
    return true;
}
...
void InitWaves(Game *g) {
    ...
    SetWave3(&w[19], ENEMY_TANK, 1, ENEMY_NORMAL, 10, ENEMY_FAST, 5, 0.5f, 3.0f, 8.0f, 300,
             "* KARANLIK LORD *");
...
void DrawFriendlyUnits(Game *g) {
    ...
    for (int i = 0; i < MAX_FRIENDLY_UNITS; i++) {
        ...
        /* Tip etiketi */
        DrawText(labels[fu->type], (int)fu->position.x - 6, (int)fu->position.y - 6, 10, BLACK);

        /* HP barı */
        float hr = fu->hp / fu->maxHp;
        DrawRectangle((int)fu->position.x - 14, (int)fu->position.y - 20, 28, 4,
                      (Color){60, 0, 0, 180});
        DrawRectangle((int)fu->position.x - 14, (int)fu->position.y - 20, (int)(28 * hr), 4,
                      (Color){30, 200, 30, 220});
...
void UpdateProjectiles(Game *g, float dt) {
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        ...
        if (p->position.x < 0 || p->position.x > SCREEN_WIDTH || p->position.y < 0 ||
...
void DrawEnemies(Game *g) {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        case ENEMY_BOSS: {
            float r = e->radius;
            float rot = e->specialTimer * 30.0f; /* Kendi etrafında yavaşça dönsün */
            DrawPoly(e->position, 6, r, rot, e->color);
            DrawPolyLines(e->position, 6, r, rot, WHITE);
            DrawCircleV(e->position, r * 0.4f, BLACK);
            DrawText(TextFormat("FAZ %d", e->bossPhase), (int)e->position.x - 14,
                     (int)e->position.y - 32, 10, WHITE);
            break;
        }
        default:
            break;
        ...
        if (e->currentHp < e->maxHp) {
            float barW = (e->type == ENEMY_BOSS) ? e->radius * 4.0f : e->radius * 2.5f;
            float barH = 4.0f;
            float barX = e->position.x - barW / 2.0f;
            float barY = e->position.y - e->radius - (e->type == ENEMY_BOSS ? 14.0f : 8.0f);
...
void DrawParticles(Game *g) {
    for (int i = 0; i < MAX_PARTICLES; i++) {
        ...
        if (!p->active)
            continue;
        DrawRectangle((int)(p->position.x - p->size / 2), (int)(p->position.y - p->size / 2),
...
void DrawPlacementPreview(Game *g) {
    /* RTS Seçim Çerçevesi Çizimi */
    if (g->isDragging) {
        Vector2 mp = GetMousePosition();
        Rectangle selRect;
        selRect.x = fminf(g->dragStartPos.x, mp.x);
        selRect.y = fminf(g->dragStartPos.y, mp.y);
        selRect.width = fabsf(mp.x - g->dragStartPos.x);
        selRect.height = fabsf(mp.y - g->dragStartPos.y);
        DrawRectangleRec(selRect, (Color){0, 255, 0, 40});
        DrawRectangleLinesEx(selRect, 1, GREEN);
    }

    if (g->selectedTowerType == TOWER_NONE)
        return;
...
void DrawHUD(Game *g) {
    ...
    if (g->waveNameTimer > 0.0f && g->waveNameText[0] != '\0') {
        ...
        snprintf(bannerBuf, sizeof(bannerBuf), "Dalga %d: %s", g->currentWave + 1, g->waveNameText);
...
void DrawGameOver(Game *g) {
    ...
    DrawBodyText(buf, (Vector2){(float)SCREEN_WIDTH / 2 - UIS(115), (float)SCREEN_HEIGHT / 2 - 20},
                 20.0f, UI_IVORY);
...
void DrawVictory(Game *g) {
    ...
    DrawBodyText(buf, (Vector2){(float)SCREEN_WIDTH / 2 - UIS(100), (float)SCREEN_HEIGHT / 2 - 20},
                 24.0f, UI_IVORY);
...
void DrawDialogueBox(Game *g) {
    ...
    if (d->visibleChars >= len)
        DrawText(">> [SPACE / Tik]", bx + bw - 160, by + bh - 22, 14, (Color){220, 200, 60, 220});
}
...
void DrawWorldMap(Game *g) {
    ...
    DrawText("Bir seviye secin", SCREEN_WIDTH / 2 - MeasureText("Bir seviye secin", 20) / 2, 72, 20,
             LIGHTGRAY);
    ...
    for (int i = 0; i < MAX_LEVELS; i++) {
        ...
        DrawCircleLines(lv->nodeX, lv->nodeY, 28, lv->unlocked ? WHITE : (Color){90, 90, 90, 200});
...
void DrawLoading(Game *g) {
    ...
    if (g->loading.tipText)
        DrawText(g->loading.tipText, SCREEN_WIDTH / 2 - MeasureText(g->loading.tipText, 16) / 2,
...
void DrawLevelComplete(Game *g) {
    ...
    if (g->currentLevel >= 0 && g->currentLevel < MAX_LEVELS)
        DrawText(g->levels[g->currentLevel].name,
                 SCREEN_WIDTH / 2 - MeasureText(g->levels[g->currentLevel].name, 26) / 2, 162, 26,
                 WHITE);
    ...
    if (!g->dialogue.active) {
        ...
        Rectangle btn = {SCREEN_WIDTH / 2.0f - 100, (float)SCREEN_HEIGHT - 120, 200, 55};
        DrawButton(btn, isLast ? "KAMPANYA BITTI!" : "Haritaya Don", isLast ? GOLD : DARKGREEN,
                   isLast ? BLACK : WHITE);
    }
}
...
#define MAX_BTN_SLOTS 32
    ...
    for (int i = 0; i < btnSlotCount; i++) {
        if (btnKeys[i].x == btn.x && btnKeys[i].y == btn.y && btnKeys[i].width == btn.width &&
            btnKeys[i].height == btn.height) {
            slot = i;
            break;
        }
    }
...
void DrawPrepPhase(Game *g) {
    ...
    DrawText("[4/5/6]: Bina Sec  |  Sol Tık: Bina Koy  |  Sag Tık: Duvar Koy  |  [D]: Dungeon  |  "
             "[SPACE]: Dalga Başlat",
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        case STATE_PLAYING:
            ...
            if (!game.waveActive && game.currentWave < game.totalWaves && IsKeyPressed(KEY_SPACE)) {
        ...
        case STATE_WAVE_CLEAR: {
            ...
            DrawText(wc, SCREEN_WIDTH / 2 - MeasureText(wc, 40) / 2, SCREEN_HEIGHT / 2 - 60, 40,
                     GOLD);
            DrawText("[ SPACE ] - Hazirlik Fazi / Sonraki Dalga", SCREEN_WIDTH / 2 - 230,
                     SCREEN_HEIGHT / 2, 24, WHITE);
            DrawText("[ D ] - Dungeon Moduna Gir", SCREEN_WIDTH / 2 - 160, SCREEN_HEIGHT / 2 + 36,
                     20, (Color){120, 220, 255, 220});
            break;
        }
        case STATE_PREP_PHASE:
            DrawPrepPhase(&game);
